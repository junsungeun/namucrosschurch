...
export default function AdminClient({ initialTemplates }: Props) {
  ...
  async function confirmDeleteTemplate() {
    ...
    await supabase.from("templates").delete().eq("id", deleteTargetId);
    ...
    <>
      <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        <div className="card-box">
          <div style={{ fontFamily: "var(--f-head)", fontSize: 14, fontWeight: 700, marginBottom: 16 }}>새 템플릿 추가</div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 120px 100px auto", gap: 12, alignItems: "end" }}>
            ...
            <div>
              <label className="label-sm">포맷</label>
            ...
            <div>
              <label className="label-sm">배경색</label>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {templates.length === 0 && (
            <p className="sub-text" style={{ textAlign: "center", padding: "40px 0" }}>
              등록된 템플릿이 없습니다
            </p>
          )}
          {templates.map((t) => (
            <div key={t.id} className="card-box" style={{ display: "flex", alignItems: "center", gap: 14 }}>
              ...
              <button onClick={() => setDeleteTargetId(t.id)} style={{ background: "none", border: "none", color: "#e05252", cursor: "pointer", fontSize: 13, padding: "5px 8px" }}>
                삭제
              </button>
            </div>
          ))}
