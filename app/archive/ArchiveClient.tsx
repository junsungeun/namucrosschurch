...
export default function ArchiveClient({ grouped }: Props) {
  ...
  async function handleUpdate(id: string) {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); alert(`수정 실패: ${d.error}`); return; }
      setEditing(null);
      router.refresh();
    } catch (e) { alert(`수정 오류: ${e}`); }
  ...
  async function handleDelete(cs: CardSet) {
    ...
    try {
      ...
      if (!res.ok) { const d = await res.json(); alert(`삭제 실패: ${d.error}`); return; }
      router.refresh();
                  ...
                  <div className="dropdown" ref={openMenu === cs.id ? menuRef : undefined}>
                    ...
                    {openMenu === cs.id && (
                      <div className="dropdown-menu">
                        ...
                        <button className="dropdown-item" onClick={() => startEdit(cs)}>✏️ 수정</button>
                        <div className="dropdown-divider" />
                        <button className="dropdown-item dropdown-item--danger" onClick={() => handleDelete(cs)}>🗑 삭제</button>
                      </div>
                    )}
                  </div>
                </div>
